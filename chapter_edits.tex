...
documentation of code. Furthermore, access to R's source code and the provision of a framework for extension
has enabled many programmers to improve on base R functionality by creating a wide variety of extensions. There are now more
than 4000 official add-on \emph{packages}, allowing it to tackle
...
All of the
results presented in this chapter can be reproduced (and modified) by inputing the short code snippets that are presented into R. Elsewhere in this book, these principles are extended in the context of reproducible geographic information science (****CB Chapter****).

\section{A practical primer on spatial data in R}

This section introduces those steps required to get started with processing spatial data in R. The focuses of the chapter is on visualisation of vector data (common in socio-economic examples), however, R also provides functionality for the analysis and visualisation of raser data (See accompanying website). As with the rest of the chapter,  a practical element is embedded, including R code that can be run on your own computer. For users
completely new to R, we would however recommend beginning with an `introduction to R'
...
we recommend new R users refer to this as accompaniment to the chapter. To
download the data that will allow the examples to be reproduced, click
on the ``Download ZIP'' button on the right hand side of the page, and unzip this to a
...
The above block of code loads the rgdal library, creates and then plots a new
\emph{object} called \texttt{wrld}. This operation should be fast on most computers because
\texttt{wrld} has quite a small file size. Spatial data can however get very large as the number and complexity of zones increases. It can therefore be useful to know the size of spatial objects and to
simplify them when necessary prior to visualisation \footnote{R makes this easy, and as described in Section
2 of the
\href{https://github.com/geocomPP/sdvwR/blob/master/sdv-tutorial.pdf?raw=true}{tutorial}
...
2013). The spatial data is divided into a series of different
...
The contents of ``slots'' within spatial data objects can be accessed using the ``\texttt{@}'' symbol. For example, you can extract attributes from the 
data slot using the square brackets to define the
subset of the data to be displayed. Row numbers are referred
...
In the above example, the first two rows of the data slot are displayed, and can be treated as a standard data frame.
...
Before progressing, we will reproject the data \footnote{For more information on referencing systems, see the supporting material on the website ********}.
The coordinate reference system of the wrld
shapefile is WGS84 with a Mercator projection (***is this right?), which is a very common latitude and longitude
format. However, from a cartographic perspective, this projection this is not ideal as it distorts
the size of countries close to the North and South poles (at the top and
bottom of the above plot). Instead, the
Robinson projection can be used to provide a better compromise between areal distortion
...
plots created in the examples thus far use R \emph{base graphics} capabilities. However, the remaining examples will use the ggplot2
package as introduced earlier. This requires the data to be in a slightly different format to base R, however, can be converted
simply using the \texttt{fortify} function. This unfortunately looses any attribute data attached to the spatial object, however,using the \texttt{merge} function 
anables this to be re-attached.
...
Now that the R object is in the correct format to be plotted with ggplot2, the following code
 produces a chropleth map coloured by the population variable. This
...
palette accordingly. The default colour palettes are generally a
sensible place to start, but users can specify them, for example, if they needed to print a  map in black and white. The
...
To change the colour scheme, these can be specified manually, either using words, as illustrated in the example, or more flexibly, such as through the use of hexadecimal colour codes \footnote{***link to a hex colour website***}.
...
Choosing appropriate colour pallets can be difficult, and there are a variety of considerations such as the intended destination of a graphic (computer screen, print etc), or
enabling graphics that are accesible to all, for example, account for potential visual impairments. As such, it is well worth looking at the \emph{Color Brewer} palettes developed
...
In addition to altering the colour pallet used to represent continuous
...
gridExtra package to display a series of maps side by side,  illustrating different break choices. ***you don't do this, but I think it would be useful***
...
\caption{Manual Break Specification}
\end{figure}

Line colour and width are further important parameters for enhancing the legibility of a graphic. The code below demonstrates it
...
its own dataset, and as such, adjust the sytax a little from that presented previously. Although more code is needed, this does enable
...
The ggmap package has been extended to enable the simple use of
...
than that used in the Google or OSM  online map services. As such, it was therefore necessary to reproject the data using the process that was illustrated earlier. 
...
This information is used to request an appropriate basemap from a selected
map tile service. The first block of code in the snippet below retrieves the
bounding box and add then 5\% so there is a little space around the edges of the data to be plotted.
...
\texttt{ggmap} and \texttt{get\_map} which are required to produce the plot and
...
\section{Case Study}
As an illustrative example, this final section presents a spatial data visualisation example that draws upon those concepts discussed in this chapter to produce a map of 18th Century Shipping flows. The data used in this visualisation have been obtained from the Climatological Database for the World's Oceans (CLIWOC) and
...
As always, the first step is to load any required packages and
datasets. The example uses the png package to load in a series of map
...
shipping routes. The \texttt{geom\_path()} function is used to amalgamate
