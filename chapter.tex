...
(using the \texttt{geosphere} package). The secret to the success of
...
the importance of such details cannot be overstated. They can be the
...
topic, see ``The structure of spatial data in R'' in the on line
...
then execute the code and generate the plot. By\\specifing the
\texttt{fill} variable within the \texttt{aes()} (short for
`aesthetics') arguement, ggplot2 colours the countries using a default
colour pallette and automatically generates a legend.
\texttt{geom\_polygon()} tells ggplot2 to plot polygons. As will be
shown in the next section these defaults can be easily altered to
...
may wish to export them as a pdf using \texttt{ggsave} function and then
add additional customisations using graphics package such as Adobe
...
opening quote. Enjoy the ability to now tweak colours and other aspects
of graphics without the need to start from scratch each time an
iterative improvement is required. As the R community grows, so will its
range of applications and available packages. The supportive online
communities surrounding large open source programs such as R are one of
their greatest assets, so we recommend you become an active ``open
source'' citizen rather than merely a passive consumer of new software
(Ramsey \& Dubovsky, 2013). There will be many exciting opportunities
